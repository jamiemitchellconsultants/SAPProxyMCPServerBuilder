...
This repository's binding AI agent instructions live in `CLAUDE.md` at the repo root. Read it in
full before generating any code or proposing any change — every rule there is binding, regardless of
which AI tool is being used.

`CLAUDE.md` is the single source of truth. This file, and the equivalent files for other tools
(`AGENTS.md`, `GEMINI.md`, `.github/copilot-instructions.md`, `.cursor/rules/`, `.clinerules/`), are
thin pointers to it — kept that way deliberately so the rules never have to be kept in sync across
multiple copies. If you are updating the instructions, edit `CLAUDE.md`, not this file.
